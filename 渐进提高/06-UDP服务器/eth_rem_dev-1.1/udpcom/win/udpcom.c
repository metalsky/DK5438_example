...
/* Note: a limitaton of the Windows version is at the moment
 * that it does not timeout and return an error if there is 
 * no answer. This is because the alarm() function is not supported
 * under windows. If you know how to do this under win32 then tell
...
#define RBLEN 500
	...
	memset(&servaddr.sin_zero, 0, sizeof(servaddr.sin_zero)); // zero fill
        ...
        if ((sockfd_s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0)
        {
                perror("open server socket failed");
                exit(1);
        }
	...
	if(sendto(sockfd_s,buf,strlen(buf)+1,0,(struct sockaddr *)&destaddr,sizeof(destaddr)) == -1){
		perror("sendto failed");
		exit(1);
	}
        closesocket(sockfd_s);
