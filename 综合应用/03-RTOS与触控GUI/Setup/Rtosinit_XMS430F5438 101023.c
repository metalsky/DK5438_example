...
#define EMBOS_TIMER_VECTOR TIMER0_A0_VECTOR
...
void OS_InitHW(void) {
  ...
  TA0CTL   = 0                           /*  Reset Timer_A5, division 1      */
           | (1 << 2)                    /*  Clear timer                     */
           | (2 << 8)                    /*  Use SMCLK as timer clock        */
//             |TASSEL_1                    // Use ACLK as timer clock
           ;
  TA0CCR0  = (OS_PCLK_TIMER / 1000) - 1; /*  Set to 1ms                      */
  TA0CCTL0 = 0                           /*  Initilize capture control       */
           | (1 << 4);                   /*  Enable compare interrupt        */
  TA0CTL  |= (1 << 4);                   /*  Start timer in UP-Mode          */
