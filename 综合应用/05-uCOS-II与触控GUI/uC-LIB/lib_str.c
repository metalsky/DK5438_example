                                               ...
                                               CPU_BOOLEAN   *pnbr_neg)
{
    ...
    if (pstr_next == (CPU_CHAR **) 0) {                         /* If NOT avail, ...                                    */
        pstr_next  = (CPU_CHAR **)&pstr_parse_next;             /* ... re-cfg NULL rtn ptr to unused local var.         */
       (void)&pstr_parse_next;                                  /* Prevent possible 'variable unused' warning.          */
    }
   *pstr_next = (CPU_CHAR *)pstr;                               /* Init rtn str for err (see Note #5).                  */

    if (pnbr_neg == (CPU_BOOLEAN *) 0) {                        /* If NOT avail, ...                                    */
        pnbr_neg  = (CPU_BOOLEAN *)&nbr_neg;                    /* ... re-cfg NULL rtn ptr to unused local var.         */
       (void)&nbr_neg;                                          /* Prevent possible 'variable unused' warning.          */
    }
